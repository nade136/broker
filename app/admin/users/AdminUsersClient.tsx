...
import { createAdminUser, deleteUserAccount } from "../actions";
...
export default function AdminUsersClient({
  ...
  initialProfiles: AdminUserProfile[];
  listError?: string | null;
}) {
  const router = useRouter();
  const [profiles, setProfiles] = useState<AdminUserProfile[]>(initialProfiles);
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteUserAccount(id, session?.access_token);
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900">
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 rounded-lg border border-gray-200 py-2 text-xs font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-60 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 rounded-lg bg-red-600 py-2 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-70 dark:bg-red-500 dark:hover:bg-red-600"
              >
